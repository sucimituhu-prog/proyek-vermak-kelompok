...
const CustomerList = () => {
  ...
  const [editData, setEditData] = useState({ nama_pelanggan: '', nomor_hp: '' });

  // 1 kartu per nomor HP unik
  const pelangganMap = {};
  orders.forEach((o) => {
    const key = o.nomor_hp || 'unknown';
    if (!pelangganMap[key]) {
      pelangganMap[key] = {
  ...
  const openEditModal = (pelanggan) => {
    ...
    setEditData({ nama_pelanggan: pelanggan.nama_pelanggan, nomor_hp: pelanggan.nomor_hp });
    setIsModalOpen(true);
  };
  ...
  const inputStyle = {
    ...
    border: '1px solid #cbd5e1', fontFamily: 'inherit', boxSizing: 'border-box',
  };
  ...
  return (
    <div style={{ padding: '30px', backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      ...
      {/* Modal Edit */}
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.55)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '20px', width: '100%', maxWidth: '450px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div>
                ...
                <input type="text" style={inputStyle} value={editData.nama_pelanggan}
                  onChange={(e) => setEditData({ ...editData, nama_pelanggan: e.target.value })} />
              </div>
              <div>
                <label style={labelStyle}>Nomor WhatsApp</label>
                <input type="text" style={inputStyle} value={editData.nomor_hp}
      ...
      <header style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#64748b', marginTop: '4px' }}>{pelangganList.length} pelanggan terdaftar</p>
      </header>
      ...
      <div style={{ position: 'relative', maxWidth: '400px', marginBottom: '24px' }}>
        ...
        <input type="text" placeholder="Cari nama atau nomor HP..."
          ...
          style={{ width: '100%', padding: '10px 12px 10px 34px', borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: '0.9rem', fontFamily: 'inherit', outline: 'none', backgroundColor: 'white' }} />
      </div>
      ...
      {pelangganList.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
          {pelangganList.map((pelanggan) => (
            <div key={pelanggan.nomor_hp} style={{ backgroundColor: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '14px' }}>

              {/* Info utama */}
              <div>
                <h3 style={{ margin: 0, color: '#1e293b' }}>{pelanggan.nama_pelanggan}</h3>
                <p style={{ color: '#64748b', fontSize: '0.9rem', margin: '4px 0 0' }}>📞 {pelanggan.nomor_hp}</p>
              </div>
                  ...
                  {pelanggan.orders.slice(0, 3).map((o) => {
                    ...
                    return (
                      <div key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: '0.7rem', fontWeight: '700', padding: '2px 8px', borderRadius: '10px', backgroundColor: cfg.bg, color: cfg.color }}>
                          {cfg.label}
                        </span>
                      </div>
                    );
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: 'auto' }}>
                <button onClick={() => openEditModal(pelanggan)}
                ...
                <button onClick={() => handleHapusSemua(pelanggan)}
